...
namespace XjjXmm.Infrastructure.Redis
{
    /// <summary>
    /// Redis操作
    /// </summary>
    public class RedisClient : IDisposable
    {
        ...
        public static string DefaultConnectionString { get; set; } = ConfigHelper.GetConfig("RedisHosts") ?? "127.0.0.1:6379,allowadmin=true,abortConnect=false,connectTimeout=1000,connectRetry=1,syncTimeout=20000";
        ...
        public RedisClient(int dbNum = 0) : this(null, dbNum)
        {
        }
        ...
        public RedisClient(string? readWriteHosts, int dbNum = 0)
        {
            DbNum = dbNum;
        ...
        /// <param name="dbNum">数据库的编号</param>
        /// <param name="_"></param>
        private RedisClient(string? readWriteHosts, int dbNum, int _)
        {
            DbNum = dbNum;
        ...
        /// <returns></returns>
        public static RedisClient GetInstance(int db = 0)
        {
        ...
        /// <param name="db">数据库的编号</param>
        /// <returns></returns>
        public static RedisClient GetSingleInstance(int db = 0)
        {
            return new RedisClient(null, db, 0);
        }
        ...
        /// <param name="db">数据库的编号</param>
        /// <returns></returns>
        public static RedisClient GetInstance(string conn, int db = 0)
        {
        ...
        /// <param name="db">数据库的编号</param>
        /// <returns></returns>
        public static RedisClient GetSingleInstance(string conn, int db = 0)
        {     
            if(redisClient == null)
            {
                redisClient = new RedisClient(conn, db, 0);
            }
            return redisClient;
        }
        ...
        public async Task<T?> GetHashAsync<T>(string key, string dataKey)
        {
           // if (KeyExists(key))
            //{
                key = AddSysCustomKey(key);
                var value = await Do(async db => await db.HashGetAsync(key, dataKey));
                return ConvertObj<T>(value);
           // }
        ...
        public IDatabase GetDatabase()
        {
            ...
            if (db == null)
            {
                throw new Exception("redis db  不存在 ");
            }
            return db;
        }
