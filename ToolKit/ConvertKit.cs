...
public static class ConvertKit
{
	...
	{
		var success = int.TryParse(input.ToSafeString(), out var result);
		if (success)
			return result;
		try
		{
	...
	{
		var success = long.TryParse(input.ToSafeString(), out var result);
		if (success)
			return result;
		try
		{
	...
	{
		var success = float.TryParse(input.ToSafeString(), out var result);
		if (!success)
			return null;
		if (digits == null)
			return result;
	...
	{
		var success = double.TryParse(input.ToSafeString(), out var result);
		if (!success)
			return null;
		if (digits == null)
			return result;
	...
	{
		var success = decimal.TryParse(input.ToSafeString(), out var result);
		if (!success)
			return null;
		if (digits == null)
			return result;
	...
	public static bool? ToBoolOrNull(object input)
	{
		...
		if (value != null)
			return value.Value;
		return bool.TryParse(input.ToSafeString(), out var result) ? (bool?)result : null;
	}
	...
	{
		switch (input.ToSafeString().ToLower())
		{
			case "0":
				return false;
			case "否":
	...
	{
		return DateTime.TryParse(input.ToSafeString(), out var result) ? (DateTime?)result : null;
	}
	...
	{
		return Guid.TryParse(input.ToSafeString(), out var result) ? (Guid?)result : null;
	}
