...
namespace XjjXmm.Infrastructure.Webs.Clients;
...
public abstract class HttpRequestBase<TRequest> where TRequest : IRequest<TRequest>
{
	...
	protected HttpRequestBase(HttpMethod httpMethod, string url)
	{
		...
		_contentType = HttpContentType.FormUrlEncoded.ToDescription();
	...
	{
		return ContentType(contentType.ToDescription());
	}
	...
	{
		_headers.Add(key, value.ToSafeString());
		return This();
	}
	...
	{
		var contentType = _contentType.ToSafeString().ToLower();
		switch (contentType)
		{
			case "application/x-www-form-urlencoded":
				return new FormUrlEncodedContent(_params.ToDictionary(t => t.Key, t => t.Value.ToSafeString()));
